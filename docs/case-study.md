...
The algorithm runs in O(V + E) time per pass. Multiple critical paths are possible and all are reported.
...
*Table: mean wall-clock milliseconds from the benchmark suite. Full results including min times and iteration counts are in `benchmark/results.txt`.*
...
A workflow-level concurrency group with `cancel-in-progress: true` ensures only the latest commit's run proceeds per PR. Branch protection rules on main require all four status checks to pass before merge.
